...
>
	...
	<div
		...
		class:pb-0={value.length > 0}
		class="border-border bg-background ring-offset-background placeholder:text-slightly-muted-foreground focus-within:ring-ring relative flex flex-wrap rounded-xl border px-3 py-2 outline-none focus-within:ring-2 focus-within:ring-offset-2 has-disabled:cursor-default has-disabled:bg-gray-200 has-disabled:opacity-50 sm:text-sm dark:has-disabled:bg-gray-800 {inputClass}"
	>
		<!--* Custom Icon *-->
		{#if !!Icon}
			<Icon
		...
		<div class="flex h-full w-full flex-wrap items-center space-x-2">
			{#if value}
		...
		<IntersectionObserver element={topDiv} bind:intersecting={topVisible}>
			...
			>
				<div class="w-full bg-red-400" bind:this={topDiv}></div>
			</div>
		...
		<IntersectionObserver element={bottomDiv} bind:intersecting={bottomVisible}>
			...
			>
				<div class="w-full bg-amber-300" bind:this={bottomDiv}></div>
			</div>
		</IntersectionObserver>
	</div>
